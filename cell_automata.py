...
###############################
# functions to generate and evolve the cellular automaton

def generate_state(size):
    '''function that returns the initial state 000010000 with length equal to size+2
...
def simulation(size, rule, iterations):
    ...
    initial_state = generate_state(size)  # generate the initial state 
...
def format_print(state, alive_symbol='*', dead_symbol='.'):
...
states_seq = simulation(size, rule, iterations)
# display the output
print("\nCellular Automaton Simulation")
print(f"Rule {rule}\n")
for i in range(len(states_seq)):
    print(format_print(states_seq[i], 'o','.'))
    time.sleep(0.5)
